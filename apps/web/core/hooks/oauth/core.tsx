...
export const useCoreOAuthConfig = (oauthActionText: string): TOAuthConfigs => {
  ...
  const oAuthOptions: TOAuthOption[] = [
    ...
    {
      id: "oidc",
      // A generic provider has no brand of its own, so the operator names it
      // in God Mode and we render that. Falls back to a neutral label.
      text: `${oauthActionText} with ${config?.oidc_display_name || "SSO"}`,
      icon: <img src={oidcLogo} height={18} width={18} alt="Single sign-on" />,
      onClick: () => {
