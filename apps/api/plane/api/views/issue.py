...
class IssueAttachmentDetailAPIEndpoint(BaseAPIView):
    ...
    def get(self, request, slug, project_id, issue_id, pk):
        ...
        # Serve safe (non-script-capable) MIME types inline so images preview in
        # the browser instead of forcing a download; script-capable types still
        # force attachment to prevent same-origin XSS.
        storage = S3Storage(request=request)
